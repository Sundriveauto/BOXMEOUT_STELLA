...
mod place_bet_security_tests {
    ...
    #[test]
    fn test_place_bet_paused_rejected() {
        let lock_threshold = SCHEDULED_AT - LOCK_BEFORE_SECS;
        let env = Env::default();
        let (client, _contract_id, factory, token_id) = setup(&env, lock_threshold - 1);
        ...
        // Pause the market via emergency_pause (only factory can call)
        client.emergency_pause(&factory);

        let result = client.try_place_bet(&bettor, &BetSide::FighterA, &1_000_000i128, &token_id);
        assert!(result.is_err(), "Paused market must reject place_bet");
    }
    ...
    fn test_place_bet_emits_event() {
        ...
        let events = env.events().all();
        let bet_placed_events = events.filter(|e| {
            let topic = e.1.get(0);
            topic == Some(Symbol::new(&env, "bet_placed"))
        });
        assert!(
            bet_placed_events.len() > 0,
            "Expected at least one bet_placed event"
        );
    }
}
